...
export default async function appStartup(
  preferencesFilePath,
  typelistFilePath,
  monthYearFilePath
) {
  ...
  try {
    const preferencesExists = await FileSystem.getInfoAsync(
      preferencesFilePath
    );
  ...
  try {
    ...
    if (!fileExists.exists) {
      ...
      await FileSystem.writeAsStringAsync(
        typelistFilePath,
        JSON.stringify(defaultTypelist)
      );
  ...
  try {
    ...
    await FileSystem.writeAsStringAsync(
      monthYearFilePath,
      JSON.stringify([fullTimeData])
    );
    console.log(`Set time: ${strMonth} [${currentMonth}], ${currentYear}.`);
  } catch (error) {
    console.error(
      `Error getting time localization for the set language (${language}):`,
      error
    );
  }
